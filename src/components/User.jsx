...
export default function User({ user_id, username, email, phone, friends, liked, imageUrl, localUser }) {
    ...
    const updateStatus = async () => {
        ...
        else if (user_id in localUser.outgoingFriendRequests) setStatus('Pending')
        else if (user_id in localUser.incomingFriendRequests) setStatus('Accept Friend')
    ...
    const handleFriendRequest = async () => {
        ...
        else if (status === 'Accept Friend' && await addFriend(localUser.user_id, user_id)) setStatus('Remove Friend')
